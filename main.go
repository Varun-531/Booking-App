	...
	for {
		...
		if userTickets <= remainingTickets {
			...
			if noTicketsRemaining {
				fmt.Println("All tickets are sold out. Check back later for more tickets")
				break
			}
		} else {
			fmt.Printf("Sorry, we only have %v tickets remaining. Please enter a number less than %v\n", remainingTickets, remainingTickets)
			continue
		}
